...
test.describe('MCQ System', () => {
  ...
  test.describe('MCQ List Page', () => {
    ...
    test('should display MCQ list page correctly', async ({ page }) => {
      ...
      // Check for MCQ cards
      const mcqCards = page.locator('[data-testid="mcq-card"], .mcq-card, .question-card');
    ...
    test('should navigate to individual MCQ', async ({ page }) => {
      ...
      // Click on first MCQ
      const firstMCQ = page.locator('[data-testid="mcq-card"], .mcq-card, .question-card').first();
  ...
  test.describe('Individual MCQ Page', () => {
    ...
    test('should display MCQ options correctly', async ({ page }) => {
      ...
      if (await firstMCQ.isVisible()) {
        ...
        // Check for options
        const options = page.locator('input[type="radio"], [data-testid="option"]');
    ...
    test('should allow selecting an answer', async ({ page }) => {
      ...
      if (await firstMCQ.isVisible()) {
        ...
        // Select an option
        const firstOption = page.locator('input[type="radio"], [data-testid="option"]').first();
    ...
    test('should submit answer correctly', async ({ page }) => {
      ...
      if (await firstMCQ.isVisible()) {
        ...
        if (await firstOption.isVisible()) {
          ...
          // Submit answer
          const submitButton = page.locator('button:has-text("Submit"), button[type="submit"], [data-testid="submit-button"]');
          if (await submitButton.isVisible()) {
            await submitButton.click();
            
            // Should show results
            await assertionHelper.assertElementVisible('text=Result, text=Correct, text=Incorrect');
          }
        }
      }
    });
      ...
      if (await firstMCQ.isVisible()) {
        ...
        if (await firstOption.isVisible()) {
          ...
          if (await submitButton.isVisible()) {
            ...
            await expect(page.locator('text=Correct, text=Incorrect, text=Right, text=Wrong')).toBeVisible();
          }
        }
      }
    });
      ...
      if (await firstMCQ.isVisible()) {
        ...
        if (await firstOption.isVisible()) {
          ...
          if (await submitButton.isVisible()) {
            ...
            await expect(page.locator('text=Explanation, text=Why')).toBeVisible();
          }
        }
      }
    });
  ...
  test.describe('MCQ Results and Progress', () => {
    ...
    test('should show MCQ progress', async ({ page }) => {
      ...
      const progressElements = page.locator('[data-testid="progress"], .progress, text=Progress');
  ...
  test.describe('Error Handling', () => {
    ...
    test('should handle MCQ loading errors', async ({ page }) => {
      ...
      await expect(page.locator('text=Error loading MCQs, text=Something went wrong')).toBeVisible();
    });
    ...
    test('should handle individual MCQ loading errors', async ({ page }) => {
      ...
      if (await firstMCQ.isVisible()) {
        ...
        // Should show error message
        await expect(page.locator('text=MCQ not found, text=Error loading question')).toBeVisible();
      }
    });
      ...
      if (await firstMCQ.isVisible()) {
        ...
        if (await firstOption.isVisible()) {
          ...
          if (await submitButton.isVisible()) {
            ...
            // Should show error message
            await expect(page.locator('text=Submission failed, text=Error submitting answer')).toBeVisible();
          }
        }
      }
    });
  ...
  test.describe('Performance', () => {
    ...
    test('should load individual MCQ within acceptable time', async ({ page }) => {
      await navigationHelper.goToMCQ();
      await pageHelper.waitForLoadingComplete();
