...
export default function InterviewResultsPage() {
  ...
  const getPerformanceLevel = (percentage: number) => {
    if (percentage >= 90) return { level: "Excellent", color: "text-green-600", bg: "bg-green-100", icon: "🏆" };
    if (percentage >= 80) return { level: "Very Good", color: "text-blue-600", bg: "bg-blue-100", icon: "🥇" };
    if (percentage >= 70) return { level: "Good", color: "text-yellow-600", bg: "bg-yellow-100", icon: "🥈" };
    if (percentage >= 60) return { level: "Satisfactory", color: "text-orange-600", bg: "bg-orange-100", icon: "🥉" };
  ...
  const correctAnswers = result.questionAnalysis.filter(q => q.correct).length;
          ...
          <Card className="p-8 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-gray-900">{Math.round(averageTimePerQuestion)}m</div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="font-semibold">
                    {result.timeSpent <= template.duration * 60 ? 'Good' : 'Slow'}
                  </span>
                </div>
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  <span className="text-gray-700">Completion Date</span>
                  <span className="font-semibold">{formatDateTime(result.completedAt)}</span>
                </div>
              </div>
            </Card>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {result.questionAnalysis.map((question, index) => {
                const templateQuestion = template.questions.find(q => q.id === question.questionId);
                return (
                  <div key={question.questionId} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <p className="text-gray-700 mb-3">{templateQuestion?.question}</p>
                    
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center space-x-4">
                        <span className={`flex items-center ${question.correct ? 'text-green-600' : 'text-red-600'}`}>
                          {question.correct ? <CheckCircle className="w-4 h-4 mr-1" /> : <XCircle className="w-4 h-4 mr-1" />}
                      ...
                      <div className="text-gray-600">
                        {Math.round((question.earnedPoints / question.points) * 100)}%
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="space-y-4">
                {result.percentage < 80 && (
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <h4 className="font-semibold text-yellow-800 mb-2">📈 Overall Performance</h4>
                    <p className="text-yellow-700 text-sm">
                      Your overall score of {result.percentage}% indicates room for improvement. 
                      Focus on understanding the core concepts better.
                    </p>
                  </div>
                )}
            ...
            <Card className="p-6">
              ...
              <div className="space-y-4">
                {Object.entries(result.categoryScores).map(([category, score]) => {
                  if (score >= 80) {
                    return (
                      <div key={category} className="p-4 bg-green-50 border border-green-200 rounded-lg">
                        <h4 className="font-semibold text-green-800 mb-2">
                          ⭐ {category.replace('-', ' ').toUpperCase()}
                        </h4>
                        <p className="text-green-700 text-sm">
                          Excellent performance with {score}%! You have strong skills in this area.
                        </p>
                      </div>
                    );
                  }
                ...
                {result.timeSpent <= template.duration * 60 && (
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 className="font-semibold text-blue-800 mb-2">⏱️ Time Management</h4>
                    <p className="text-blue-700 text-sm">
                      Great job managing your time efficiently! You completed the interview within the allocated time.
                    </p>
                  </div>
                )}
              </div>
